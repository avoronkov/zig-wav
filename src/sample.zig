...
pub fn convert(comptime T: type, value: anytype) T {
    ...
    if (S == u8) {
        const value8: i8 = @bitCast(value -% 128);
        return convert(T, value8);
    } else if (T == u8) {
        const uvalue8: u8 = @bitCast(convert(i8, value));
        return uvalue8 +% 128;
    }
...
fn convertIntToFloat(comptime T: type, value: anytype) T {
    const S = @TypeOf(value);
    const maxt: T = @floatFromInt(std.math.maxInt(S));
    const valuet: T = @floatFromInt(value);
    return 1.0 / (1.0 + maxt) * valuet;
}
